...
module.exports = {
	...
	updateMessagesList: function(messages) {
		...
		var log_rows = log.lpos.height - 2;

		for(var i=0; i < Math.min(messages.length, log_rows); i++) {
			var message = messages[i];
			log.log(moment(message.timestamp).format() + " - " + message.source + " - " + message.message.trim())
		}
