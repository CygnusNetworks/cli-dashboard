  ...
  lastMessagesOfStream: function(options, callback) {
    ...
    request({uri: url, json: true, qs: parameters}, function (error, response, body) {
      ...
      if (!error && response.statusCode == 200) {
        ...
        if(response == undefined) {
          var message = "Could not get messages of stream. Error: " + error;
        } else {
          var message = "Could not get messages of stream. Error: HTTP " + response.statusCode;
        }
        result.push({timestamp: Date.now(), source: 'cli-dashboard', message: message});
        callback(result);
      }
    }).auth(options.username, options.password, false)
  },
